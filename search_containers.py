...
def search_barcodes(csvfile, csvoutfile, api_url, headers):
    for row in csvfile:
        barcode = row[0]
        try:
            ...
            newrow = [barcode, series, identifier, title, container_profile, container_number, location_title]
...
def main():
    ...
    csv_headers = ['barcode', 'series', 'identifier', 'title', 'container_profile', 'container_number', 'location']
    csvoutfile.writerow(csv_headers)
    print('\nPlease wait a moment...')
    search_barcodes(csvfile, csvoutfile, api_url, headers)
    fileobject.close()
    keeptime(starttime)
